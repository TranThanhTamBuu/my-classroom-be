...
export class LinkService {
      ...
      if (aClass.teachers.includes(user._id.toString())) {
        ...
        if (inviteEmail && inviteEmail.length > 0) {
          ...
          inviteEmail.forEach((val) => {
            ...
            this.mailService.sendMailInviteClass(
              val.toString(),
              inviteLink,
  ...
  async joinInvitationLink(
    joinInvitationLink: JoinInvitationLink,
    user: Users,
  ): Promise<any> {
    ...
    if (aLink.isPrivateLink) {
      // private link
      if (
        Date.now() >= aLink.expiredTime ||
        !aLink.inviteEmail.includes(user.email.toString())
      ) {
        throw new NotAcceptableException(
          'You are not invited to this class or invite link is expired',
        );
      }
    }

    if (
      aLink.emailConstrain != null &&
      !user.email.toString().includes(aLink.emailConstrain.toString())
    ) {
      throw new NotAcceptableException(
        'You are not invited to this class or invite link is expired',
      );
  ...
  async checkAuthAcceplink(linkId: string, user: Users): Promise<any> {
    ...
    if (
      aLink.emailConstrain &&
      email.includes(aLink.emailConstrain.toString())
    ) {
      return Promise.resolve({
        success: true,
        isAccepted: true,
